...
function scanForMocks(dirPath, mockDirectory, app, resourcesExposed) {
	var dirContents = fs.readdirSync(dirPath);

	// we look for config.json
	if (_.contains(dirContents, "config.json")){
		setUpMockedResource.setUpMockedResource(dirPath, mockDirectory, app, resourcesExposed);
	}
	...
	directories.forEach(function(directory){
		scanForMocks(path.join(dirPath, directory), mockDirectory, app, resourcesExposed);
	});
  }